...
def collect(
    ...
    *,
    ...
    command_name = _command_name(observation, notes)
...
def _command_name(observation: ProcessObservation, notes: list[str]) -> str:
    """The command_name recorded as evidence: secret free, and still matchable.
    ...
    that gets shared. So it is redacted on the way IN, not on the way out,
    because the record outlives whatever was going to filter it later.

    Redaction alone was too costly though. A redacted joined argv matches no
    manifest command, so the decision silently became ungradeable, which trades a
    privacy bug for a correctness one. When redaction actually changed something,
    this therefore falls back to argv[0] alone: an executable name is essentially
    never a secret, and the evaluator's matching rule already accepts argv[0], so
    a wrapped `npm test --token sk-...` still grades against a manifest declaring
    `npm test` while the secret never reaches the file. When redaction changed
    nothing, the full joined form is kept, because it is strictly more useful.

    The fallback is itself passed through `redact`, which is a no-op for any real
    executable name. That is not weakening the rule to make matching work: it is
    refusing to write out an unredacted string on the one path where argv[0]
    itself is the secret, where being ungradeable is the correct outcome.
    """
    joined = observation.command_name
    redacted = redact(joined) or joined
    if redacted == joined:
        return joined

    head = observation.command[0] if observation.command else joined
    safe_head = redact(head) or head
    notes.append(
        "a secret-shaped value in the wrapped command was redacted, so "
        f"command_name records {safe_head!r} (argv[0]) instead of the full "
        "command line; a manifest naming argv[0] still matches it"
    )
    return safe_head
